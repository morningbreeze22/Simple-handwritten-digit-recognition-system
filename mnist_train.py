...
def train(mnist):
    ...
    with tf.Session() as sess:
        ...
        ckpt = tf.train.get_checkpoint_state("./model") #获取checkpoints对象
        latest=tf.train.latest_checkpoint("./model")
        i=int(latest.split("-")[1])
        #print(i)
        if ckpt and ckpt.model_checkpoint_path:##判断ckpt是否为空，
            saver.restore(sess,ckpt.model_checkpoint_path)#恢复保存
